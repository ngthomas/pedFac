...
simFulldata <- function(seed.num = 234,
  ...
  outfolder.path <- paste0(outdir.path,"/sf_",sf.rate,"/",replicate.indx)
  ...
  maxLL.ped.tbl <- maxLL.ped %>% ungroup() %>%
    mutate(kid.id =  subbingID(kid, max.id, id.ls), 
           pa.id = subbingID(pa, max.id, id.ls),
           ma.id = subbingID(ma, max.id, id.ls))
  ...
  run.summary$greedy.fullsib.cluster.ls <- RetrieveFullSibGrp(out.ped.tbl, mating.factor.tbl, max.id, id.ls)
...
RunSequoia <- function(seed.num = 234,
  ...
  if (sf.rate != 0) {      
  ParOUT_Mono <- sequoia(GenoM = as.matrix(geno.matrix), LifeHistData = lifeHistory.tbl, MaxSibIter = 0,Complex = "mono")}
  ...
  if (sf.rate != 0) {
    ...
    run.summary$parent.tbl <- left_join(parentage.grp.truth, ParOUT_Mono$PedigreePar %>% as_tibble() %>% select(id, sire, dam, LLRpair) %>% ungroup() %>% mutate(id =as.numeric(id)) , by="id") %>%
      ungroup() %>%
...
RunColony <- function(seed.num = 234,
  ...
  system(colony.prep.cmd,ignore.stderr = F, ignore.stdout = F)
  
  dir.create(paste0(outfolder.path,"/colony"), recursive = TRUE)
  file.copy(paste0(outfolder.path,"/colony_0.in"),
  ...
  colony.run.files <- list.files(".", pattern = paste0(run.name))
  file.copy(colony.run.files, paste0(outfolder.path, "/colony/."), recursive = T, copy.date=T, overwrite = T) 
...
sib.pairs.ls <- colony.cluster.tbl %>%
  ...
  full.sib.pair.infer <- left_join(sib.pairs.ls, colony.cluster.tbl %>% separate_rows(fullsib) %>% rename(kid.1 = "fullsib"), by= "kid.1")
...
fullsib.pedfac <- lapply(1:5, function(sf) {
   lapply(1:5, function (rep) {
    simFulldata(replicate.indx = rep, pedigraph.version = 0.181, pedfac.opt= " -j 1 -f 10", n.iter = 100, seed.num = seed.num[rep], sf.rate = sf.array[sf])
  })
})

save(fullsib.pedfac, file = "/Users/thomasn/repo/pedigree-factor-graphs/data/ch2/fullsib/pedfac_full.Rda")
...
map(.x = c(1:4), .f = function(x) {
fullsib.pedfac[[x]][[1]]$parent.prob.tbl
fullsib.COLONY[[x]][[1]]$parent.tbl
fullsib.sequoia[[x]][[1]]$parent.tbl
})
