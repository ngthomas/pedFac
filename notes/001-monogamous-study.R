...
# load
load("/Users/thomasn/repo/pedigree-factor-graphs/data/ch2/fullsib/COLONY_full_medPrior.Rda")
fullsib.COLONY.medPrior <- fullsib.COLONY
load("/Users/thomasn/repo/pedigree-factor-graphs/data/ch2/fullsib/COLONY_full_rep.Rda")
fullsib.COLONY <- fullsib.COLONY.rerun
load("/Users/thomasn/repo/pedigree-factor-graphs/data/ch2/fullsib/sequoia_full.rerun.Rda")
fullsib.sequoia <- fullsib.sequoia.rerun
load("/Users/thomasn/repo/pedigree-factor-graphs/data/ch2/fullsib/pedfac_full.Rda")
load("/Users/thomasn/repo/pedigree-factor-graphs/data/ch2/fullsib/pedfac_w_swap.Rda")
#load("/Users/thomasn/repo/pedigree-factor-graphs/data/ch2/fullsib/pedfac_w_swap_sub.Rda")
load("/Users/thomasn/repo/pedigree-factor-graphs/data/ch2/fullsib/pedfac_w_swap_sub.rerun.Rda")
fullsib.pedfac_w_swap_sub <- fullsib.pedfac_w_swap_sub.rerun
#load("/Users/thomasn/repo/pedigree-factor-graphs/data/ch2/fullsib/pedfac_w_swap_sub100.Rda")
...
Longo <- expand_grid(
  ...
  mutate(
    ...
    colony_no_sibprior = map2(.x = samp_frac_idx, .y = rep.indx, .f = function(x, y) fullsib.COLONY[[x]][[y]]),
    colony_med_sibprior = map2(.x = samp_frac_idx, .y = rep.indx, .f = function(x, y) fullsib.COLONY.medPrior[[x]][[y]]),
    sequoia = map2(.x = samp_frac_idx, .y = rep.indx, .f = function(x, y) fullsib.sequoia[[x]][[y]]),
    pedFac_plain = map2(.x = samp_frac_idx, .y = rep.indx, .f = function(x, y) fullsib.pedfac[[x]][[y]]),
    pedFac_w_swap = map2(.x = samp_frac_idx, .y = rep.indx, .f = function(x, y) fullsib.pedfac_w_swap[[x]][[y]]),
    pedFac_w_swap_sub = map2(.x = samp_frac_idx, .y = rep.indx, .f = function(x, y) fullsib.pedfac_w_swap_sub[[x]][[y]])
  ) %>%
  pivot_longer(
    cols = colony_no_sibprior:pedFac_w_swap_sub,
    names_to = "software",
    values_to = "output"
  )
...
compiled.parentage.tbl <- Longo %>% filter(sampl.frac != 0) %>%
  ...
  mutate(
    ...
    parent_tbl_prob = case_when(
      software %in% c("pedFac_plain", "pedFac_w_swap","pedFac_w_swap_sub") ~ map(output, "parent.prob.tbl"),
...
prob.rank.plot.tbl <- parentage.stat %>% #filter(!software %in% c("pedFac_w_swap","pedFac_w_swap_sub")) %>%
  mutate(software = factor(software, levels=c("pedFac_plain","pedFac_w_swap","pedFac_w_swap_sub",  "COLONY", "sequoia"), labels = c("pedFac"," ... (+swap)"," ... (+swap,+sub)",  "COLONY", "sequoia")),
...
ggsave(paste0("notes/fig/ch2_fullsib_indxplot_",i,".pdf"), parentage.prob.plot,  device="pdf", height=5, width=9, units="in", dpi=500)


parentage.prob.ggplot[[i]] <- grid.arrange(parentage.prob.plot, legend.parentage, ncol=1,heights=c(10.5,1),
                                    padding = unit(0, "line"))
ggsave(paste0("notes/fig/ch2_fullsib_indxplot_",i,".png"), parentage.prob.ggplot[[i]],  device="png", height=5, width=9, units="in", dpi=500)
...
parentage.stat.tbl <- parentage.stat %>%
  mutate(software = factor(software, levels=rev(c("pedFac_plain","pedFac_w_swap","pedFac_w_swap_sub",  "COLONY", "sequoia")), labels = rev(c("pedFac","  ... (+swap)","  ... (+swap,+sub)",  "COLONY", "sequoia"))),
         sampl.frac = factor(sampl.frac, levels=c(1,0.75,0.5, 0.25), labels = c("sf:\n1.0","sf:\n0.75","sf:\n0.5","sf:\n0.25"))) %>%
  arrange(rank, desc(prob)) %>%
  group_by(software, rep.indx, sampl.frac) %>%
...
parentage.stat.plot.tbl <- parentage.stat.tbl %>%
  mutate(
    stat = case_when(
...
g <- ggplot(data=parentage.stat.plot.tbl) +
  geom_rect(data = subset(parentage.stat.plot.tbl, sampl.frac %in% c("sf:\n1.0","sf:\n0.5")), aes(xmin = -Inf,xmax = Inf, ymin = -Inf,ymax = Inf), alpha = 0.01, fill="#AED6F1") +
  ...
  geom_point(data= parentage.boxstat.tbl %>%
               mutate(
                 stat = case_when(
                   stat=="AUC"~"Area Under the Curve",
                   stat=="AR"~"Precision"
...
ggsave("notes/fig/ch2_fullsib_stat.pdf", g, device="pdf", height=5, width=9, units="in", dpi=500)

ggsave("notes/fig/ch2_fullsib_stat.png", g,  device="png", height=5, width=9, units="in", dpi=500)
...
parentage.prep.xtable <- parentage.boxstat.tbl %>% ungroup() %>%
  mutate(
    software = factor(software, levels=c("pedFac","  ... (+swap)","  ... (+swap,+sub)",  "COLONY", "sequoia")),
...
column_spec(1, bold = T, width = "4em", latex_valign= "m", latex_column_spec = "c") %>%
  #column_spec(0, latex_valign= "c") %>%
...
compiled.sibship.pairwise.tbl <- Longo %>%
  ...
  mutate(
    software = case_when(software == "colony_no_sibprior"~"COLONY",
                         ...
                         TRUE ~ software),
    FS_pairwise_tbl = map(output, "pairwise.tbl"),
...
for (i in 1:5) {
  FS.prob.plot <-
    ...
    scale_x_continuous("Full Sibling (FS) Pairs Index")+
    theme(#strip.placement = "outside",
      strip.background.y = element_rect(fill= "dark grey"),
      strip.background.x = element_rect(fill= "dark grey"),
      strip.text.y.left = element_text(angle = 0,colour = "white"),
  ...
  ggsave(paste0("notes/fig/ch2_fullsib_SibPair_indxplot_",i,".pdf"), FS.prob.plot, device="pdf", height=5, width=9, units="in", dpi=500)

  ggsave(paste0("notes/fig/ch2_fullsib_SibPair_indxplot_",i,".png"), FS.prob.ggplot[[i]],  device="png", height=5, width=9, units="in", dpi=500)
...
FS.ROC.ggplot <- ggplot(roc.FS.compiled.df %>% arrange(fpr, tpr), aes(x = fpr, y = tpr)) +
  ...
  ggsave(paste0("notes/fig/ch2_fullsib_SibPair_ROCplot.pdf"), FS.ROC.ggplot, device="pdf", height=5, width=9, units="in", dpi=500)
...
for (i in 1:5) {
  ...
  ggsave(paste0("notes/fig/ch2_fullsib_trueSibgrp_",i,".pdf"), true.sib.group.plot, device="pdf", height=5, width=9, units="in", dpi=500)

  ggsave(paste0("notes/fig/ch2_fullsib_trueSibgrp_",i,".png"), true.sib.grp.ggplot[[i]],  device="png", height=5, width=9, units="in", dpi=500)
}
...
colorBrewer2_scale <- c('#e41a1c','#377eb8','#4daf4a','#984ea3','#ff7f00','#ffff33')
...
for (i in 1:5) {
  ...
  ggsave(paste0("notes/fig/ch2_fullsib_inferSibgrp_",i,".pdf"), infer.sib.group.plot, device="pdf", height=5, width=9, units="in", dpi=500)

  ggsave(paste0("notes/fig/ch2_fullsib_inferSibgrp_",i,".png"), infer.sib.grp.ggplot[[i]],  device="png", height=5, width=9, units="in", dpi=500)
}
