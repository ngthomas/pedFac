---
title: '# pedfac --- factor-graph based Bayesian pedigree inference'
...
`pedfac` is an R-based front-end for a C-based computational program called `pedigraph`.
Currently, `pedfac` is available from GitHub, and, in the process of installing the package,
the binary `pedigraph` for Mac OSX will be installed too.
...
genotype frequencies. If genotype posterior values are given, the `gerror` information is ignored. If the genotype is not specified, we default the genotype error baseline as 0.02. 
