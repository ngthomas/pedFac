...
latex.out <- kbl(parentage.prep.xtable, booktabs = T, "latex") %>%
  column_spec(1, bold = T, width = "5em", latex_valign= "m", latex_column_spec = "c") %>%
...
write(latex.out.1,  "notes/table/ch2_poly_parent_stat_table.txt")
...
Plot.SibPair <- function(sib.label = "fullsib") {
...
compiled.sibship.pairwise.tbl <- Longo %>%
  mutate(
    software = case_when(software == "colony_no_sibprior"~"COLONY",
                         ...
                         TRUE ~ software),
    FS_pairwise_tbl = map(output, paste0(sib.label, ".pairwise.tbl")),
...
FS.infer.tbl <- compiled.sibship.pairwise.tbl %>%
  ungroup() %>%
...
for (i in 1:5) {
  ...
  if (sib.label == "halfsib") {
    legend.FS <- legend.FS +
      geom_text(aes(x=0.68, y=0.6, label= "False HS Assignment"),size=3.5, hjust="left", vjust="middle")
  } else {
    legend.FS <- legend.FS +
...
write(latex.out.2, paste0("notes/table/poly_",sib.label,"pairTable1.txt"))
...
halfsib.prep.xtable <- Plot.SibPair("halfsib")

allsib.pre.xtable <- left_join(
fullsib.prep.xtable %>% select(-FDR_range, -FNR_range, -AUC_range) %>%
...
latex.out <- kbl(allsib.pre.xtable, booktabs = T, "latex") %>%
  ...
  add_header_above(c(" ", " ", "Assigned Full-Sibling Pair" = 3, "Assigned Half-Sibling Pair" = 3), bold= T) %>%
  landscape()

#latex.out.2 <- gsub('(AUC\\\\_)|(FDR\\\\_)|(FNR\\\\_)|(HS\\\\_)|(FS\\\\_)', "", latex.out, perl = T)
latex.out.1 <- gsub('(HS\\\\_)|(FS\\\\_)', "", latex.out, perl = T)
latex.out.2 <- gsub('Nada', "\\\\multicolumn{1}{c}{-}", latex.out.1, perl = T)

write(latex.out.2, paste0("notes/table/poly.allsib.pairTable1.txt"))
...
for (i in 1:5) {
  ...
  ggsave(paste0("notes/fig/ch2_poly_trueSibgrp_",i,".pdf"), true.sib.group.plot, device="pdf", height=5, width=9, units="in", dpi=500)

  ggsave(paste0("notes/fig/ch2_poly_trueSibgrp_",i,".png"), true.sib.grp.ggplot[[i]],  device="png", height=5, width=9, units="in", dpi=500)
}
...
for (i in 1:5) {
  ...
  ggsave(paste0("notes/fig/ch2_poly_inferSibgrp_",i,".pdf"), infer.sib.group.plot, device="pdf", height=5, width=9, units="in", dpi=500)

  ggsave(paste0("notes/fig/ch2_poly_inferSibgrp_",i,".png"), infer.sib.grp.ggplot[[i]],  device="png", height=5, width=9, units="in", dpi=500)
}
...
write(cluster.latex.out.2, "notes/table/poly_FS.cluster.Table.txt")
